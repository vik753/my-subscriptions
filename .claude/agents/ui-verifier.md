...
1. The caller names the screen/state and the route or steps to reach it, plus the reference screenshot(s) from `design_handoff_my_subscriptions/screenshots/`.
2. Write a throwaway Playwright script in the scratchpad (not in the repo) that opens the app (`npm run preview` or the dev server the caller gives), reaches the state, and captures:
